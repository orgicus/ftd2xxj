...
JNIEXPORT jint JNICALL Java_com_ftdichip_ftd2xx_Device_getBitMode
(JNIEnv *env, jclass deviceClass, jlong handle)
  {
...
(JNIEnv *env, jclass deviceClass, jlong handle, jbyteArray buffer, jint offset, jint length)
  {
    ...
    FT_HANDLE ftHandle = ToNativeHandle(handle);
    FT_STATUS result = FT_Read(ftHandle, nativeBuffer, (DWORD)length, &numBytesRead);
...
JNIEXPORT void JNICALL Java_com_ftdichip_ftd2xx_Device_setDeadmanTimeout
(JNIEnv *env, jclass device, jlong handle, jlong timeout)
  {
    FT_HANDLE ftHandle = ToNativeHandle(handle);
    FT_STATUS result = FT_OK;

    result = FT_SetDeadmanTimeout(ftHandle, (DWORD)timeout);

    Logger_Fine("(%s: %i) - FT_SetDeadmanTimeout(%X, %u): %i", __FILE__, __LINE__,
        (unsigned int)ftHandle, (unsigned int)timeout, (int)result);

    HandleError(env, result);
  }

JNIEXPORT jlong JNICALL Java_com_ftdichip_ftd2xx_Device_getComPortNumber
(JNIEnv *env, jclass device, jlong handle)
  {    
    FT_STATUS result = FT_OK;
...
#ifdef _WINDOWS
	FT_HANDLE ftHandle = ToNativeHandle(handle);
    result = FT_GetComPortNumber(ftHandle, &comPortNumber);

    Logger_Fine("(%s: %i) - FT_GetComPortNumber(%X, %u): %i", __FILE__, __LINE__,
        (unsigned int)ftHandle, (unsigned long)comPortNumber, (int)result);
#else
    result = FT_NOT_SUPPORTED;
    Logger_Severe("(%s: %i) - FT_GetComPortNumber() is not supported on this OS.", __FILE__, __LINE__);
#endif
