...
public final class Service {
	...
	private static void initializeProperties() {
		...
		try {
			PropertyPersistenceManager
					.initializeFromSystemProperties(Service.class);
		} catch (Exception e) {
	...
	static void configureDevice(Device device) throws FTD2xxException {
		...
		if (usbReceiveBufferSize >= 0 && usbTransmitBufferSize >= 0) {
			logger.config(String.format("usbReceiveBufferSize: %d",
					usbReceiveBufferSize));
			logger.config(String.format("usbTransmitBufferSize: %d",
		...
		if (resetPipeRetryCount >= 0) {
			logger.config(String.format("resetPipeRetryCount: %d",
					resetPipeRetryCount));
	...
	/**
	 * Forces a rescan of the USB for FTDI devices. It can be of use when trying
	 * to recover devices programatically.
	 * 
	 * @throws FTD2xxException
	 *             if something went wrong scanning the USB.
	 */
	 ...
	 * zero, the drivers for USB root hubs will be reloaded, causing all USB
	 * devices connected to reload their drivers.
	 * 
	 * @param vid
	 *            vendor ID.
	 * @param pid
	 *            product ID.
	 * @throws FTD2xxException
	 *             if the driver can not be reloaded.
	 */
	public static native void reload(int vid, int pid) throws FTD2xxException;
}
