...
/**
 * Represents the Device to access a particular the ft2xx device (e.g. FT232BM). <br>
 * <br>
...
public class Device {
	...
	/**
	 * Creates a new <code>Device</code>. The device descriptor is read from the
	 * EEPROM.
	 * 
	 * @param locationID
	...
	public ModemStatus getModemStatus() throws FTD2xxException {
		...
		return ModemStatus.valueOf(getModemStatus(handle));
	}
	...
	/**
	 * Retrieves the current value of the bit mode for this device. Predefined  
	 * values can be retrieved using {@link BitBangMode#valueOf(int)}.
	 * 
	...
	public int getBitMode() throws FTD2xxException {
		...
		return getBitMode(handle);
	}

	/**
	 * Retrieves the current value of the bit mode of a particular device.
	 * 
	 * @param handle
	 *            the native handle of the device.
	 * @return the mode value.
	 ...
	 *             if the mode can not be retrieved.
	 */
	private static native int getBitMode(long handle)
			throws FTD2xxException;
	 ...
	 * @throws FTD2xxException
	 *             if the timeout can not be set.
	 */
	public void setDeadmanTimeout(long timeout) throws FTD2xxException {
		checkIsOpen();

		if (timeout < 0 || timeout > 0xFFFFFFFF)
			throw new IllegalArgumentException(Localizer.getLocalizedMessage(
					Device.class, "error.invalidDeadmanTimeout", timeout));

		setDeadmanTimeout(handle, timeout);
	}
	 ...
	 *             if the timeout can not be set.
	 */
	private static native void setDeadmanTimeout(long handle, long timeout)
			throws FTD2xxException;
	 ...
	 *            the buffer to store the data in.
	 * @param offset
	 *            the index within the <code>buffer</code> of the first byte to
	 *            store.
	 * @param length
	 ...
	 * @throws FTD2xxException if the COM port number can not be retrieved.
	 */
	public long getComPortNumber() throws FTD2xxException {
		checkIsOpen();

		return getComPortNumber(handle);
	}

	private static native long getComPortNumber(long handle)
			throws FTD2xxException;
}
